...
const Navbar = () => {
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    ripple.style.left = `${e.clientX - link.getBoundingClientRect().left - radius}px`;
    ripple.style.top = `${e.clientY - link.getBoundingClientRect().top - radius}px`;
  ...
  return (
    <nav
      className={`fixed ${roboto.className} top-2 sm:top-4 inset-x-2 sm:inset-x-4 lg:inset-x-0 mx-auto w-[calc(100%-1rem)] sm:w-[calc(100%-2rem)] lg:max-w-4xl bg-black/80 backdrop-blur-md shadow-lg rounded-full border border-purple-900/50 px-3 sm:px-4 lg:px-6 py-2 sm:py-3 lg:py-4 flex flex-wrap justify-around items-center text-base sm:text-lg lg:text-xl font-bold z-50 transition-all duration-300`}
    >
